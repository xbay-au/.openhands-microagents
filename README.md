...
These are the openhands microagets used by myself. The ones generated by me have the suffix a_ to keep them at the top of the list.
...
## FFUF Agent Wrapper Script

A new tool-based microagent `ffuf_agent.py` has been added. It provides an interactive, wizard-style interface for:

- Scan-type selection (directory discovery, API fuzzing, subdomains, etc.)
- Automatic wordlist management (downloads missing SecLists files)
- Custom headers, proxy, delays, recursion options
- JSON output parsing and summary tables
- Session history for rerunning and comparison
...
1. Install dependencies:
   ...
   ```
2. Ensure `ffuf` is installed and in your PATH.
3. Run the agent:
   ```bash
